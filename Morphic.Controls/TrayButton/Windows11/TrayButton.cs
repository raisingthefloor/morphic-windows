...
internal class TrayButton : IDisposable
{
    ...
    private Microsoft.UI.Dispatching.DispatcherQueueTimer? _reattemptShowTaskbarButtonTimer = null;
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disposedValue)
        {
            if (disposing)
            {
                ...
                _reattemptShowTaskbarButtonTimer?.Stop();
                ...
                case TrayButtonVisibility.Visible:
                    if (_visibility == TrayButtonVisibility.Hidden)
                    {
                        ...
                        if (showResult.IsError == true)
                        {
                            ...
                            // NOTE: we use a DispatcherQueueTimer here (instead of a system timer) so that the callback is invoked on the UI thread
                            var dispatcherQueue = Microsoft.UI.Dispatching.DispatcherQueue.GetForCurrentThread();
                            _reattemptShowTaskbarButtonTimer = dispatcherQueue.CreateTimer();
                            _reattemptShowTaskbarButtonTimer.Interval = REATTEMPT_SHOW_TASKBAR_BUTTON_INTERVAL_TIMESPAN;
                            _reattemptShowTaskbarButtonTimer.IsRepeating = true;
    ...
    // NOTE: we use a DispatcherQueueTimer here instead of a system timer so that the .Show() call runs on the UI thread
    private void ReattemptShowTaskButtonTimer_Tick(Microsoft.UI.Dispatching.DispatcherQueueTimer sender, object args)
    {
