...
{
   public record CaptureSelectedTextError : MorphicAssociatedValueEnum<CaptureSelectedTextError.Values>
   {
       // enum members
       public enum Values
       {
   ...
   public static MorphicResult<string?, CaptureSelectedTextError> GetSelectedText()
   {
   ...
   private static MorphicResult<string?, CaptureSelectedTextError> GetSelectedTextUsingUIAutomation(CUIAutomation8 comUIAutomation)
   {
   ...
   private static MorphicResult<string?, CaptureSelectedTextError> QueryElementForSelectedText(IUIAutomationElement comUIAutomationElement)
   {
       // NOTE: at this point, we should have the handle of an element which we can query for selected text
   ...
   private static MorphicResult<string?, CaptureSelectedTextError> ExtractStringFromTextRangeArray(IUIAutomationTextRangeArray comUIAutomationTextRangeArray)
   {
       // NOTE: at this point, we have a COM-instantiated IUIAutomationTextRangeArray (text range array); now let's extract each text range from this array, convert it to a string, and then concatenate all those ranges together into a string to return to our caller
