...
public class Package
{
    public interface IGetPackageNameError
    {
        ...
        public record InvalidPackageIdentity : IGetPackageNameError; // NOTE: this should never occur (but could occur if GetCurrentPackageFullName returns success with a zero-byte package name)
        //
        public record UnknownWin32Error(uint Win32Error) : IGetPackageNameError;
    }
    public static MorphicResult<string, IGetPackageNameError> GetPackageName()
    {
        ...
        switch (getCurrentPackageFullNameResult)
        {
            case Windows.Win32.Foundation.WIN32_ERROR.APPMODEL_ERROR_NO_PACKAGE:
                ...
                return MorphicResult.ErrorResult<IGetPackageNameError>(new IGetPackageNameError.NoPackageIdentity());
            ...
            default:
                ...
                //
                TypeValidationUtils.AssertIfNotEnumOfType(getCurrentPackageFullNameResult, typeof(uint), TypeValidationUtils.ASSERT_MESSAGE_API_RESULT_TYPE_MISMATCH);
                return MorphicResult.ErrorResult<IGetPackageNameError>(new IGetPackageNameError.UnknownWin32Error((uint)getCurrentPackageFullNameResult));
        }
        ...
        switch (getCurrentPackageFullNameResult)
        {
            case Windows.Win32.Foundation.WIN32_ERROR.ERROR_SUCCESS:
                if (packageFullNameLength <= 1)
                {
                    // NOTE: just in case the package name returned is an empty result, we are returning false in that scenario; this would presumably never happen in production
                    Debug.WriteLine("WARNING: GetCurrentPackageFullName returned success, but the packageFullName argument returned a null string");
                    return MorphicResult.ErrorResult<IGetPackageNameError>(new IGetPackageNameError.InvalidPackageIdentity());
                }
                ...
                return MorphicResult.OkResult(packageFullName);
            case Windows.Win32.Foundation.WIN32_ERROR.APPMODEL_ERROR_NO_PACKAGE:
                // the process has no package identity
                return MorphicResult.ErrorResult<IGetPackageNameError>(new IGetPackageNameError.NoPackageIdentity());
            case Windows.Win32.Foundation.WIN32_ERROR.ERROR_INSUFFICIENT_BUFFER:
    ...
    {
        var getPackageNameResult = Package.GetPackageName();
        if (getPackageNameResult.IsSuccess)
        {
            return MorphicResult.OkResult(true);
        }

        switch (getPackageNameResult.Error!)
        {
            case IGetPackageNameError.NoPackageIdentity:
