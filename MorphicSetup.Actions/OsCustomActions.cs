...
public static class OsCustomActions
{
    ...
    // Launches the just-installed Morphic.exe (with --run-after-install) at the end of a fresh
    // install, via ShellExecute. We use a managed CA + ShellExecute rather than a type-18 EXE custom
    // action (which starts the target with CreateProcess) because Morphic ships with uiAccess=true,
    // and a uiAccess executable CANNOT be launched by CreateProcess from the msiexec context (it
    // fails, typically with ERROR_ELEVATION_REQUIRED / 740). Process.Start with UseShellExecute=true
    // is the launch path that DOES start a uiAccess app -- the same path the old WixShellExec custom
    // action used -- and unlike WixShellExec it can also pass the --run-after-install argument (so the
    // post-install launch respects the persisted bar visibility, like a logon autostart).
    //
    // Scheduled immediate + Impersonate=yes after InstallFinalize (see Package.wxs), so it runs in the
    // installing user's session and Morphic appears on their desktop. Best-effort: a launch failure is
    // logged but never fails the install (matches the old asyncNoWait behavior).
    [CustomAction]
    public static ActionResult LaunchMorphic(Session session)
    {
        try
        {
            // [#morphic_exe] resolves to Morphic.exe's installed full path (on disk by now; this runs
            // after InstallFinalize). This is an immediate CA, so it can read the session directly.
            string morphicExePath = session.Format("[#morphic_exe]");
            if (string.IsNullOrEmpty(morphicExePath))
            {
                session.Log("LaunchMorphic: could not resolve [#morphic_exe]; skipping launch.");
                return ActionResult.Success;
            }

            session.Log("LaunchMorphic: launching \"" + morphicExePath + "\" --run-after-install via ShellExecute");
            var startInfo = new System.Diagnostics.ProcessStartInfo
            {
                FileName = morphicExePath,
                Arguments = "--run-after-install",
                // ShellExecute path: REQUIRED to launch the uiAccess=true exe (CreateProcess fails).
                UseShellExecute = true,
            };
            using (System.Diagnostics.Process.Start(startInfo))
            {
                // fire-and-forget; disposing the wrapper does not terminate the launched process.
            }
            return ActionResult.Success;
        }
        catch (Exception ex)
        {
            // Never fail the install over a best-effort launch (matches the old asyncNoWait behavior).
            session.Log("LaunchMorphic: failed to launch Morphic; install still succeeds. Details: " + ex.GetType().FullName + ": " + ex.Message);
            return ActionResult.Success;
        }
    }
}
