...
namespace MorphicService
{
    static class UserService
    {
        ...
        {
            var request = HttpRequestMessageExtensions.Create(service.Session, string.Format("users/{0}", identifier), HttpMethod.Get);
            return await service.Session.Send<User>(request);
        }
        ...
        {
            var request = HttpRequestMessageExtensions.Create(service.Session, string.Format("users/{0}", user.Id), HttpMethod.Put, user);
